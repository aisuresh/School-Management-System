...
class Student extends Controller 
{
	...
	function studentlist1()
	{
		...
		/*
		if($section == 'All'){
					//select all sections for a slected class			
					$table = 'sectionclass';
					$where = array(
					...
					if(count($section) > 0){
						foreach($section as $row){
							$sectionall =$row->SectionId.",".$sectionall;
						}
					}
					if(count($sectionall) > 0){
						$sectionall = $sectionall.substr($sectionall, 0, count($sectionall)-2);
					}
					...
					$tjoin = array(
						...
						array('TableName' => 'course' , 'Condition' => 'studentclass.StudentClassId=course.ClassId'),
		...
		$result = $this->Adminmodel->show_rows($table, $where);
		 if(!isset($result) && $result <= 0){
			...
			if($status){
				...
				//print_r($newstudentid);exit;
				foreach($newstudentid as $sid){
					if($sid->StudentId != NULL){
	...
	function section(){			
		if($this->input->post('ClassId') == 'All'){
			$str1 = "<option value = 'x'>- - Select Section - -</option>";
			$str2 =$str2. "<option value = 'All'>ALL</option>";
			$str = $str1.$str2;
			echo $str;
			exit;
		}
	...
	function newassign(){
		...
		$this->pageinfo['event'] ='Assign New Class';
		$this->load->view('header', $this->pageinfo);
		$this->load->view('menu');
		$table = 'course';		
		$where = array(
		...
		if(count($year) > 0) {
			$acdamicyearsplit = explode('-', $year);
			$data['nextacadamicyear'] = ($acdamicyearsplit[0]+1)."-".($acdamicyearsplit[1]+1); 
		}
				 ...
				 { 
                    ...
                    case 1: // StudentRollNo
					 ...
					 case 2:
					   if($data->sheets[0]['cells'][$i][$j] == ''){
						 ...
						 $j = $data->sheets[0]['numCols'];
						 $i = $data->sheets[0]['numRows'];
					   }else if(!preg_match('/^([A-Za-z]+)$/', $data->sheets[0]['cells'][$i][$j])){
					     $validation = false;
						 $j = $data->sheets[0]['numCols'];
						 $i = $data->sheets[0]['numRows']; 		
						}else{
					   $datainfo[$info[$j]] = $data->sheets[0]['cells'][$i][$j];
					   $studentname = $datainfo[$info[$j]];	}
					...
					case 7:
					   if($data->sheets[0]['cells'][$i][$j] == ''){
						...
						$Bdate = $datearray[0].'-'.$datearray[1].'-'.$datearray[2];
						$DateOfBirth = date("Y-m-d", strtotime($Bdate));
						$datainfo[$info[$j]] = $DateOfBirth;
					   }
					...
					case 9:
					 ...
					 case 12:
					   if($data->sheets[0]['cells'][$i][$j] == 'Select'){
					     ...
					     if($data->sheets[0]['cells'][$i][$j] == 'Indian'){
						   $datainfo[$info[$j]] = 1;
						 }else if($data->sheets[0]['cells'][$i][$j] == 'Russian'){
						   $datainfo[$info[$j]] = 2;
				         }
					   }
					 ...
					 case 13:
					   if($data->sheets[0]['cells'][$i][$j] == 'Select'){
					     ...
					     if($data->sheets[0]['cells'][$i][$j] == 'Tamil'){
							...
							$datainfo[$info[$j]] = 2;
						 }else if($data->sheets[0]['cells'][$i][$j] == 'Telugu'){
							$datainfo[$info[$j]] = 3;
						 }else if($data->sheets[0]['cells'][$i][$j] == 'Kanada'){
							$datainfo[$info[$j]] = 4;
						 }else if($data->sheets[0]['cells'][$i][$j] == 'Malayalam'){
							$datainfo[$info[$j]] = 5;
						 } 
					   }
					 ...
					 case 14:
					   if($data->sheets[0]['cells'][$i][$j] == 'Select'){
					    ...
					    if($data->sheets[0]['cells'][$i][$j] == 'Yes'){
						  $datainfo[$info[$j]] = 'Y';
					 ...
					 case 16:
					   ...
					   break;
					 case 17:
					  // print($data->sheets[0]['cells'][$i][$j]);
					   if($data->sheets[0]['cells'][$i][$j] == ''){
						  $validation = false;
						 $j = $data->sheets[0]['numCols'];
						 $i = $data->sheets[0]['numRows'];   
					   }else if(!preg_match('/^\d{10}$/', $data->sheets[0]['cells'][$i][$j])){
					     $validation = false;
						 $j = $data->sheets[0]['numCols'];
						 $i = $data->sheets[0]['numRows'];  
						}else{
					 ...
					 if($data->sheets[0]['cells'][$i][$j] == ''){
						 ...
						 $j = $data->sheets[0]['numCols'];
						 $i = $data->sheets[0]['numRows'];   
					   }else if(!preg_match('/([\w\-]+\@[\w\-]+\.[\w\-]+)/', $data->sheets[0]['cells'][$i][$j])){;
					     $validation = false;
						 $j = $data->sheets[0]['numCols'];
						 $i = $data->sheets[0]['numRows']; 	
						}else{
					 ...
					 case 20:
					   if($data->sheets[0]['cells'][$i][$j] == 'Select'){
					     ...
					     if($data->sheets[0]['cells'][$i][$j] == 'A1+'){
							$datainfo[$info[$j]] = 1;
						 }else if($data->sheets[0]['cells'][$i][$j] == 'A1-'){
							$datainfo[$info[$j]] = 2;
						 }else if($data->sheets[0]['cells'][$i][$j] == 'A2+'){
							$datainfo[$info[$j]] = 3;
						 }else if($data->sheets[0]['cells'][$i][$j] == 'A2-'){
							$datainfo[$info[$j]] =4;
