...
echo "<a href='".$url."hostelfees/export' >";
...
echo "<div class='events'>";
echo "<a href='".$url."hostelfees/addnew' >";
...
foreach( array_reverse($this->session->userdata('years')) as $ys){
	if($this->session->userdata('yearhf') == $ys->AcademicYear){
...
echo '<tr><th colspan = "4" id = "table_title">Hostel Fees List View </th></tr>';
...
echo "<td align='center' ><input type='radio' name = 'HostelFeeId' id = 'HostelFeeId' value ='" . $row->HostelFeeId. "'/></td>";
...
function edit_data(){
   ...
   if(year != currentAcademicYear ){
	...
	var rollid = $('input:radio[name=HostelFeeId]:checked').val();
	if(rollid == 'undefined' || rollid == null ){
		message('Please select any one record' );
	} else {
	 window.location='<?=$url?>hostelfees/edit/'+rollid;
	}
	}
  ...
  var currentAcademicYear = $('#currentAcademicYear').val();
   if(year != currentAcademicYear ){
	...
	if(rollid == 'undefined' || rollid == null ){
	 ...
	 window.location='<?=$url?>hostelfees/view/'+rollid;
	}
	}
}	
...
function addnew_data(){	
   ...
   if(year != currentAcademicYear ){
	...
	window.location='<?=$url?>hostelfees/addnew/';
	}
	
}
function get_list(){
	var year = $('#Year').val();
	window.location='<?=$url?>hostelfees/listview/'+year;
}		
