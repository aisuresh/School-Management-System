...
$teacherlist = $this->Adminmodel->query_fun($sql);
	...
	foreach($daylist as $dl){
			...
			echo'<tr>
				...
				for($k = 0; $k < $total_periods; $k++){
					
					$sql = "select timetable.ClassId, timetable.PeriodId, timetable.SubjectId, timetable.TeacherId from timetable where timetable.Year = '$year' and  timetable.ClassId = '$cl->ClassId' and timetable.PeriodId = $k and timetable.DayId = '$dl->DayId'";
					$timetablelist = $this->Adminmodel->query_fun($sql);
					if(isset($timetablelist)){
						foreach($timetablelist as $ttl){ 
								
								$sql = "select subject.SubjectId, subject.SubjectName from facultylist 
								join subject on facultylist.SubjectId = subject.SubjectId
								where facultylist.TeacherId = '$ttl->TeacherId'"; 
								$sublist = $this->Adminmodel->query_fun($sql);
								
								if($k % 2 != 0 ){
									echo'<td>';
										echo '<select name = "teacher'.$a.$i.$k.'" id ="teacher'.$a.$i.$k.'" class = "timetable_select" onChange = "show_subjects('.$a.','.$i.','.$k.','.$ttl->SubjectId.');" >';
											echo '<option value = "x" >-- Teacher --</option>';
											foreach($teacherlist as $tl){
												if($tl->TeacherId == $ttl->TeacherId ){
													echo '<option value = "'.$tl->TeacherId.'" selected >'.$tl->StaffName.'</option>';
												}else{
										...
										echo '<select name = "subject'.$a.$i.$k.'" id ="subject'.$a.$i.$k.'" class = "timetable_select" style ="margin-top:10px;" >';
											...
											foreach($sublist as $sl){
												if($sl->SubjectId == $ttl->SubjectId){
													echo '<option value = "'.$sl->SubjectId.'" selected>'.$sl->SubjectName.'</option>';	
												}else{
													echo '<option value = "'.$sl->SubjectId.'" >'.$sl->SubjectName.'</option>';	
												}											
											}
										echo'</select>';
									echo'</td>';
...
function show_subjects(a,i,k, subject){
