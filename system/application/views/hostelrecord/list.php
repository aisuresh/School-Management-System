...
echo "<ul>";
echo "<li> <label> <input type='radio' name='searchfield' id ='searchfield' value='StudentRollNo' /> Roll No </label> </li>";
echo "<li> <label> <input type='radio' name='searchfield' id ='searchfield' value='StudentName' />Student Name</label> </li>";
echo "<li> <label> <input type='radio' name='searchfield' id ='searchfield' value='PaidBy' /> Paid By </label> </li>";
echo "<li> <label> <input type='radio' name='searchfield' id ='searchfield' value='TermPaidDate' /> Term Paid Date </label> </li>";
echo "</ul>";
echo "</li>";
echo "</ul>";
echo "</span>";
echo "<span class='search_btn'> <a onClick='search_fun(this);' id = 'hostelrecord' > <img src='".$url ."images/search_btn.gif' /> </a> </span>";	
echo "</div>";
...
echo "<a href='".$url."hostelrecord/export' >";
...
foreach( array_reverse($this->session->userdata('years')) as $ys){
	if($this->session->userdata('yearhr') == $ys->AcademicYear){
...
echo '<tr><th colspan = "9" id = "table_title">Hostel Record List View </th></tr>';
echo "<tr>";
echo "<th align='center' >S.No</th>";
echo "<th align='center' > <input type = 'hidden' name = 'tablename' class='hostelrecord' id ='ordertype'  value = '0' /> </th>";
echo "<th align='center' >Roll No <a class = 'RollNo' onClick='sort_fun(this);' >
											<img id='updown_arrow' class = 'hostelrecord.RollNo' src=''.$url.'images/up_down_arrow.gif' />
											<img id='desc' class='hostelrecord.RollNo'  src='".$url."images/order_down.gif' /> 
											<img id='asc' class='hostelrecord.RollNo' src='".$url."images/ORDER_UP.GIF' />
</th>";
echo "<th align='center' >Student Name <a class = 'StudentName' onClick='sort_fun(this);' >
											...
											<img id='asc' class='student.StudentName' src='".$url."images/ORDER_UP.GIF' />
</th>";
echo "<th align='center' >Hostel Term No</th>";
echo "<th align='center' >Hostel Term Fee</th>";
echo "<th align='center' >Term Paid Date<a class = 'HostelTermFee' onClick='sort_fun(this);' >
											<img id='updown_arrow' class = 'hostelrecord.HostelTermFee' src=''.$url.'images/up_down_arrow.gif' />
											<img id='desc' class='hostelrecord.HostelTermFee'  src='".$url."images/order_down.gif' /> 
											<img id='asc' class='hostelrecord.HostelTermFee' src='".$url."images/ORDER_UP.GIF' /></th>";
echo "<th align='center' >Paid By<a class = 'PaidBy' onClick='sort_fun(this);' >
											<img id='updown_arrow' class = 'hostelrecord.PaidBy' src=''.$url.'images/up_down_arrow.gif' />
											<img id='desc' class='hostelrecord.PaidBy'  src='".$url."images/order_down.gif' /> 
											<img id='asc' class='hostelrecord.PaidBy' src='".$url."images/ORDER_UP.GIF' /></th>";
...
echo "<td align='center' ><input type='radio' name = 'HostelRecordId' id = 'HostelRecordId' value ='" . $row->HostelRecordId . "'/></td>";
...
echo "<td align='center'>" . $TermPaidDate . "</td>";
...
function edit_data(){
   ...
   if(year != currentAcademicYear ){
    ...
    var rollid = $('input:radio[name=HostelRecordId]:checked').val();
  ...
  var currentAcademicYear = $('#currentAcademicYear').val();
   if(year != currentAcademicYear ){
	...
	if(rollid == 'undefined' || rollid == null ){
	 ...
	 window.location='<?=$url?>hostelrecord/view/'+rollid;
	}
	}
}	
function addnew_data(){	
   ...
   if(year != currentAcademicYear ){
	...
	window.location='<?=$url?>hostelrecord/addnew/';
	}
...
function get_list(){
	var year = $('#Year').val();
	window.location='<?=$url?>hostelrecord/listview/'+year;
}	
</script>
